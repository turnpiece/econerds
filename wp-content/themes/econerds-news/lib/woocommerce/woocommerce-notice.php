 ...
 * Econerds News.
 *
 * This file adds the Genesis Connect for WooCommerce notice to the Econerds News Theme.
 *
 * @package News
 * @author  StudioPress
 * @license GPL-2.0+
...
function news_woocommerce_theme_notice() {
	...
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'econerds-news' ), esc_html( CHILD_THEME_NAME ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		), __( 'install and activate Genesis Connect for WooCommerce', 'econerds-news' ) );

		$notice_html = sprintf( __( 'Please %s to <strong>enable WooCommerce support for %s</strong>.', 'econerds-news' ), $install_link, esc_html( CHILD_THEME_NAME ) );
	}
